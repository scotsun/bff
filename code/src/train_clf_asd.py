...
def get_args():
    ...
    parser.add_argument(
        "--lr", type=float, default=5e-4, help="float: learning rate; default 5e-4"
    )
    parser.add_argument(
        "--embeddingMode",
        type=str,
...
def main():
    ...
    CONTRA_MOD = MultiModalSNN(DEVICE) if args.addContrastive else None
    ZP_ONLY = args.zpOnly
    ...
    dataset_list = [
        generate_yearly_data(
            "asd", i, train_cohort, transform, MODALITY_CONFIG, complete_case=False
        )
    ...
    model_path = "../../model_checkpoint/asd/" + "_".join(model_tags)
    ...
    log_path = "../../model_result/asd/" + "_".join(model_tags) + ".json"
